...
public class Login_userController implements Initializable {
    ...
    private SingnIn user;
    public static int idUsrer;
    ...
    private void loginButtonAction(ActionEvent event) throws IOException, SQLException, ClassNotFoundException {
        if (tfUserName.getText().isEmpty() || password.getText().isEmpty()) {
            ...
            ArrayList<SingnIn> list = SingnIn.getAllPatientUser();
            int count = 0;
            String name = "";
            for (int i = 0; i < list.size(); i++) {
                if (list.get(i).getEmail().equals(tfUserName.getText()) && list.get(i).getPassword().equals(password.getText())) {
                    ...
                    idUsrer = list.get(i).getId();
                    count++;
                }
            }
            if (count > 0) {
                ViewMangeer.closeLoginPage();
                ViewMangeer.openDashBordUserPage();
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
    ...
    @FXML
    private void loginAdminAction(ActionEvent event) throws IOException {
